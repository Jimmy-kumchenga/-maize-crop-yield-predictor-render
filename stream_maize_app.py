...
st.write("### Enter farm details below to estimate **maize yield in bags (50 kg each)**")
...
# Simplified dropdowns for rainfall and temperature
...
if st.button("Predict Yield"):
    try:
        predicted_kg = model.predict(input_data)[0]
        predicted_bags = predicted_kg / 50
        st.success(f"🌾 Estimated Maize Yield: **{predicted_bags:.2f} bags/ha** (50kg bags)")
    except Exception as e:
        st.error(f"❌ Error during prediction: {e}")

st.markdown("---")
st.caption("Built with ❤️ for Malawian Farmers | Powered by AI & Agriculture 🌱")
