...
# Malawi districts and their approximate central latitude and longitude coordinates
districts_coords = {
...
def get_forecast(lat, lon, api_key):
    ...
    data = response.json()
    # Get the next forecasted interval (usually the next 3 hours)
    forecast = data['list'][0]
    temperature = forecast['main']['temp']
    rainfall = forecast.get('rain', {}).get('3h', 0)
    forecast_time = forecast['dt_txt']
    return temperature, rainfall, forecast_time
...
if st.button("Fetch Forecast Weather Data"):
    try:
        temperature, rainfall, forecast_time = get_forecast(lat, lon, api_key)
        st.success(f"District: {district}\nForecast time: {forecast_time}\nForecast Temperature: {temperature}°C\nForecast Rainfall (next 3h): {rainfall} mm")
    except Exception as e:
        st.error("Could not fetch forecast weather data. Please check your internet connection and API key.")
else:
...
if temperature is None or rainfall is None:
    st.warning("Please click the 'Fetch Forecast Weather Data' button to get forecasted temperature and rainfall for your district.")
else:
    st.write(f"Using forecast data: Temperature={temperature}°C, Rainfall={rainfall} mm (next 3 hours)")
